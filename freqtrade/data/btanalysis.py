...
def load_backtest_data(filename: Union[Path, str]) -> pd.DataFrame:
    ...
    df["open_time"] = pd.to_datetime(
        df["open_time"], unit="s", utc=True, infer_datetime_format=True
    )
    df["close_time"] = pd.to_datetime(
        df["close_time"], unit="s", utc=True, infer_datetime_format=True
    )
...
def analyze_trade_parallelism(results: pd.DataFrame, timeframe: str) -> pd.DataFrame:
    ...
    dates = [
        pd.Series(pd.date_range(row[1].open_time, row[1].close_time, freq=f"{timeframe_min}min"))
...
def load_trades(
    source: str, db_url: str, exportfilename: Path, no_trades: bool = False
) -> pd.DataFrame:
    """
...
def extract_trades_of_period(
    ...
    trades = trades.loc[
        (trades["open_time"] >= trades_start) & (trades["close_time"] <= trades_stop)
    ]
    return trades
