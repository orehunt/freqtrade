...
def generate_text_table_strategy(
    stake_currency: str, max_open_trades: str, all_results: Dict
) -> str:
    """
...
def show_backtest_results(
    config: Dict, btdata: Dict[str, DataFrame], all_results: Dict[str, DataFrame]
):
    for strategy, results in all_results.items():
    ...
    if len(all_results) > 1:
        ...
        table = generate_text_table_strategy(
            config["stake_currency"], config["max_open_trades"], all_results=all_results
        )
