...
class Backtesting:
    ...
    def _get_close_rate(
        self, sell_row, trade: Trade, sell: SellCheckTuple, trade_dur: int
    ) -> float:
        """
        ...
        if sell.sell_type in (SellType.STOP_LOSS, SellType.TRAILING_STOP_LOSS):
            ...
            if roi is not None:
                ...
                close_rate = -(
                    trade.open_rate * roi + trade.open_rate * (1 + trade.fee_open)
                ) / (trade.fee_close - 1)
                if (
                    trade_dur > 0
    ...
    def _get_sell_trade_entry(
        ...
        for sell_row in partial_ohlcv:
            if max_open_trades > 0:
                ...
                trade_count_lock[sell_row.date] = (
                    trade_count_lock.get(sell_row.date, 0) + 1
                )
