...
class FreqtradeBot:
    ...
    def exit_positions(self, trades: List[Any]) -> int:
        ...
        for trade in trades:
            try:
                ...
                if trade.open_order_id is None and self.handle_trade(
                    trade, trades_signals[trade.pair]
                ):
                    trades_closed += 1
    ...
    def handle_trade(self, trade: Trade, signals: Tuple[bool, bool] = None) -> bool:
        ...
        if config_ask_strategy.get("use_order_book", False):
            ...
            order_book = self._order_book_gen(
                trade.pair,
                f"{config_ask_strategy['price_side']}s",
            ...
            for i in range(order_book_min, order_book_max + 1):
                ...
                logger.debug(
                    f"  order book {config_ask_strategy['price_side']} top {i}: "
                    f"{sell_rate:0.8f}"
                )
    ...
    def handle_stoploss_on_exchange(self, trade: Trade) -> bool:
        ...
        try:
            ...
            stoploss_order = (
                self.exchange.get_order(trade.stoploss_order_id, trade.pair)
    ...
    def handle_timedout_limit_buy(self, trade: Trade, order: Dict) -> bool:
        ...
        trade.amount = order["amount"] - safe_value_fallback(
            corder, order, "remaining", "remaining"
        )
    ...
    def update_trade_state(
        self, trade: Trade, action_order: dict = None, order_amount: float = None
    ) -> bool:
        """
    ...
    def get_real_amount(self, trade: Trade, order: Dict, order_amount: float = None) -> float:
        ...
        if (
            ...
            if (
                order["fee"]["currency"] is not None
                and order["fee"]["cost"] is not None
                and trade_base_currency == order["fee"]["currency"]
            ):
        ...
        trades = self.exchange.get_trades_for_order(
            trade.open_order_id, trade.pair, trade.open_date
        )
        ...
        for exectrade in trades:
            ...
            if (
                "fee" in exectrade
                and exectrade["fee"] is not None
                and (exectrade["fee"].keys() >= {"currency", "cost"})
            ):
                # only applies if fee is in quote currency!
                if (
                    exectrade["fee"]["currency"] is not None
                    and exectrade["fee"]["cost"] is not None
                    and trade_base_currency == exectrade["fee"]["currency"]
                ):
        ...
        if fee_abs != 0:
            logger.info(
                f"Applying fee on amount for {trade} "
