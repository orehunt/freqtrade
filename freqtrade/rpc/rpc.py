...
class RPC:
    ...
    def _rpc_trade_status(self) -> List[Dict[str, Any]]:
        ...
        if not trades:
            ...
            for trade in trades:
                ...
                fmt_close_profit = (
                    f"{round(trade.close_profit * 100, 2):.2f}%" if trade.close_profit else None
                )
    ...
    def _rpc_trade_statistics(
        ...
        profit_closed_percent = (
            round(mean(profit_closed_ratio) * 100, 2) if profit_closed_ratio else 0.0
        )
        profit_closed_fiat = (
    ...
    def _rpc_blacklist(self, add: List[str] = None) -> Dict:
        ...
        if add:
            ...
            for pair in add:
                if (
                    self._freqtrade.exchange.get_pair_quote_currency(pair) == stake_currency
