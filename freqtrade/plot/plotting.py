...
def add_max_drawdown(
    fig, row, trades: pd.DataFrame, df_comb: pd.DataFrame, timeframe: str
) -> make_subplots:
    """
...
def generate_candlestick_graph(
    ...
    *,
    ...
    if "buy" in data.columns:
        ...
        if len(df_buy) > 0:
            buys = go.Scatter(
                ...
                mode="markers",
                name="buy",
                marker=dict(symbol="triangle-up-dot", size=9, line=dict(width=1), color="green",),
            )
    ...
    if "sell" in data.columns:
        ...
        if len(df_sell) > 0:
            sells = go.Scatter(
                ...
                mode="markers",
                name="sell",
                marker=dict(symbol="triangle-down-dot", size=9, line=dict(width=1), color="red",),
            )
    ...
    if "bb_lowerband" in data and "bb_upperband" in data:
        ...
        if (
            "bb_upperband" in plot_config["main_plot"]
...
def plot_profit(config: Dict[str, Any]) -> None:
    ...
    trades = trades[
        (trades["pair"].isin(plot_elements["pairs"])) & (~trades["close_time"].isnull())
    ]
