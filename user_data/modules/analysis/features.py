...
def direction(df, histLen=1):
    ...
    return pd.DataFrame(
        data=df_dir.values, index=df.index, columns=["direction_{n}".format(n=histLen)]
    )
...
def bollinger_bandwidth(df, histLen=10):
    ...
    return pd.DataFrame(
        data=v_bb, index=df.index, columns=["bollinger_bw_{m}".format(m=histLen)]
    )
...
def close_ma_ATR_log(df, histLen=5, atrLen=30):
    ...
    return pd.DataFrame(
        data=v, index=df.index, columns=["cmatr_{n}_{m}".format(n=histLen, m=atrLen)]
    )
...
def ATR(df, atrLen=10):
    ...
    return pd.DataFrame(
        data=l_atr, index=df.index, columns=["atr_{n}".format(n=atrLen)]
    )
...
def atr_per_close(df, atrLen=10):
    ...
    l = a / df[["close"]].values
    return pd.DataFrame(
        data=l.values, index=df.index, columns=["atr_per_close_{n}".format(n=atrLen)]
    )
...
def ATRRatio(df, shortLen=10, longLen=20):
    ...
    return pd.DataFrame(
        data=v_atr_ratio,
        index=df.index,
        columns=["ATR_Ratio_{n}/{m}".format(n=shortLen, m=longLen)],
    )
...
def RSI(df, field="open", histLen=3):
    ...
    return pd.DataFrame(
        data=lst_rsi, index=df.index, columns=["rsi_{n}".format(n=histLen)]
    )
...
def Trend(df, histLen=50):
    ...
    return pd.DataFrame(
        data=delta, index=df.index, columns=["trend_open_sma_{n}".format(n=histLen)]
    )
...
class TrendIndicators:
    ...
    def DecyclerOsc(data, HPPeriod1=30, HPPeriod2=60):
        ...
        alpha1 = (
            np.cos(0.707 * 2 * np.pi / HPPeriod1)
            + np.sin(0.707 * 2 * np.pi / HPPeriod1)
            - 1
        ) / np.cos(0.707 * 2 * np.pi / HPPeriod1)
        alpha2 = (
            np.cos(0.707 * 2 * np.pi / HPPeriod2)
            + np.sin(0.707 * 2 * np.pi / HPPeriod2)
            - 1
        ) / np.cos(0.707 * 2 * np.pi / HPPeriod2)
    ...
    def Decycler(data, cutoff=60):
        ...
        """
        alpha1 = (np.cos(2 * np.pi / cutoff) + np.sin(2 * np.pi / cutoff) - 1) / np.cos(
            2 * np.pi / cutoff
        )
        # Close = df[['close']].values
        ...
        for i in range(len(decycle)):
            if i < 1:
                ...
                decycle[i] = (alpha1 / 2) * (data[i] + data[i - 1]) + (
                    1 - alpha1
                ) * decycle[i - 1]
    ...
    def ZMA(data, period=100):
        ...
        for i in range(len(zma)):
            if i < period - 1:
                ...
                zma[i] = (
                    a * (ema[i] + best_gain * (data[i] - zma[i - 1]))
                    + (1 - a) * zma[i - 1]
                )
    ...
    def SuperSmooth(data, period=50):
        ...
        for i in range(len(data)):
            if i < 2:
                ...
                ss[i] = (
                    c1 * (data[i] + data[i - 1]) * 0.5 + c2 * ss[i - 1] + c3 * ss[i - 2]
                )
