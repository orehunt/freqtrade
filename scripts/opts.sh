...
while getopts "ta:d:q:e:i:p:f:xm:ctd:s:n:g:u:l:r:y:o:v:b:j" o; do
    case "$o" in
        ...
        u) userdir=$OPTARG ;;
        m) dmmp="--dmmp"; opt_move=$OPTARG; ;;
        o) open_trades=$OPTARG ;;
        s) eps="--eps"; opt_save=$OPTARG; spread=$OPTARG; ;;
        c) clear=true; config=true ;;
        n) tuned_amounts=$OPTARG ;;
        l) sqlite=$OPTARG; opt_arc=1 ;;
        r) cross_data=$OPTARG; plot_type=1; runtime_config=$OPTARG ;;
        z) testcond=$OPTARG ;;
        y) strategy_name=$OPTARG ;;
        v) runmode=$OPTARG ; opt_cv=$OPTARG ;;
